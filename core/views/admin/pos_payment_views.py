"""Admin POS payment session endpoints."""
...
from core.views.admin.resource_views import _forbidden, validation_error
...
def admin_pos_payment_methods_status(request):
...
@authentication_classes([TokenAuthentication, SessionAuthentication])
@permission_classes([IsAuthenticated])
def admin_pos_nchl_qr_session_create(request):
    if err := _forbidden(request):
        return err
...
@authentication_classes([TokenAuthentication, SessionAuthentication])
@permission_classes([IsAuthenticated])
def admin_pos_payment_session_detail(request, session_id: str):
    if err := _forbidden(request):
        return err
    try:
        session = pos_payment_service.get_session_for_user(session_id, request.user, vendor=None)
...
@authentication_classes([TokenAuthentication, SessionAuthentication])
@permission_classes([IsAuthenticated])
def admin_pos_nchl_qr_session_confirm_demo(request, session_id: str):
    if err := _forbidden(request):
        return err
    try:
        session = pos_payment_service.get_session_for_user(session_id, request.user, vendor=None)
...
@authentication_classes([TokenAuthentication, SessionAuthentication])
@permission_classes([IsAuthenticated])
def admin_pos_esewa_session_create(request):
    if err := _forbidden(request):
        return err
    if not pos_checkout_allowed():
        return pos_disabled_response()
